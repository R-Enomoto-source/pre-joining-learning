...
`exercise/Chapter2/Chapter2_sec.md` に書かれていた解説・原則を、復習用に題目／コツ形式で整理したものです。分野の切り分けと用語の正確さのために、`local_problems/answers/Chapter2_answers.md`（第2章の公式解答・解説）の設問番号を併記しています。各コツの下に**最小のコード例**を付けています。
...
for (String s : list) {
    System.out.println(s); // A, B のあと B を remove すると C が繰り上がり、
    if ("B".equals(s)) {
        list.remove(s);    // 拡張forのカーソルが進むタイミングで C が飛ばされ得る
    }
}
```
