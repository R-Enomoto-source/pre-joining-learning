# 第3章（Chapter3_sec）— 間違えないためのコツまとめ

`weaknessesReview/weaknessesReview.md` の弱点整理を出発点にしつつ、**第3章の設問 1〜43 すべて**について「間違えないコツ」を**題目／コツ形式**でまとめています。用語と設問対応の正確さのために、`local_problems/answers/Chapter3_answers.md`（第3章の公式解答・解説）の設問番号を併記しています。各コツの下に**最小のコード例**を付けています。断片のため **`package` や必要な `import` は試験・ローカル実行に合わせて補ってください**。

---

**項目**

演算子 — **複合代入**と連鎖代入の評価順（`Chapter3_answers.md` 設問1）

**題目**

`b = a += 5` のように、**複合代入が先に完結してから**別変数へ代入される流れを取り違えないため

**コツ**

**`a += 5`** は「`a` を読む → 足す → **`a` に書き戻す**」までが一式で、その**式の値は更新後の `a`**。その結果が **`b = …`** の右辺になる。**`b = a += 5` は `a += 5` のあと `b = a`** と読む。

**例**

```java
int a = 3, b;
b = a += 5; // a は 8、b も 8
System.out.println(a + b); // 16
```

---

**項目**

数値・型 — リテラル既定とコンパイルエラーになる代入（`Chapter3_answers.md` 設問2）

**題目**

「エラーになる行」を選ぶ問題で、**本当にエラーになる組み合わせだけ**を選び切るため

**コツ**

整数リテラルは原則 **`int`**、浮動小数リテラルは原則 **`double`**。`byte` / `short` へは**値が範囲内**なら `int` リテラルでも暗黙変換が認められるが、**範囲外**は不可。`int` と **`long`（`L`）** の演算は **`long`** になり、結果を **`int` 変数**へ代入しようとするとエラー。**`double` → `float`** は暗黙変換できない。設問で「エラーにならない行」として **`int` 同士の演算が `short` に収まる`**パターンが出たら、それは**エラーではない**（弱点一覧でも触れている取りこぼしポイント）。

範囲

| 型 | サイズ | 最小値 | 最大値 | 備考 |
|----|--------|--------|--------|------|
| `byte` | 8 bit | -128 | 127 | バイナリデータ、小規模な数値 |
| `short` | 16 bit | -32,768 | 32,767 | メモリ制限がある場合の小規模数値 |

**例**

```java
// byte b = 0b1000_0000; // int リテラル 128 → byte へは暗黙変換 NG
byte b = (byte) 0b1000_0000; // 明示キャストなら可

// int x = 6L * 1; // long → int NG
long y = 6L * 1;

// float f = 1.0; // double リテラル → float NG
float g = 1.0f;

short s = 10 * 10 / 10; // int 演算だが値が short に収まれば代入可（「エラーでない」パターンの代表）
```

---

**項目**

演算子 — **前置／後置**の `++` `--`（`Chapter3_answers.md` 設問3）

**題目**

1 つの式に複数の増減があるとき、**いつの値が式に使われるか**を取り違えないため

**コツ**

**前置**（`++a`）は先に変数を更新し、**更新後の値**が式に使われる。**後置**（`a++`）は**古い値**で式のその部分を評価し、**そのあと**で変数を更新。式の中は演算子の出現順に従い、**都度「今の変数の値」**を当てはめる。

**例**

```java
int a = 10;
int x = ++a; // a=11, x=11
int y = a++; // y=11（古い a）、その後 a=12
```

---

**項目**

演算子 — **関係演算子**と `boolean`（`Chapter3_answers.md` 設問4）

**題目**

`boolean` に **`>` `>=` `<` `<=`** を使えると誤解しないため

**コツ**

**大小比較**（`>` `>=` `<` `<=`）の左右は**数値的な大小**が前提。**`boolean` には使えずコンパイルエラー**。**等価**（`==` `!=`）は `boolean` にも使える。

**例**

```java
boolean a = true, b = false;
// boolean c = a <= b; // NG
boolean ok = a != b; // OK
```

---

**項目**

演算子 — **`&&` `||` のショートサーキット**（`Chapter3_answers.md` 設問5）

**題目**

左が決まった時点で**右が評価されない**ことによる、変数の副作用の有無を取り違えないため

**コツ**

**`&&`**: 左が **`false` なら右は評価しない**（結果は確定）。**`||`**: 左が **`true` なら右は評価しない**。**`&` `|`** は（boolean でも）**両方評価**（非ショート）。試験では **`++` が右に付いている**パターンで落とし穴になりやすい。

**例**

```java
int b = 10;
boolean r = (10 < 10) && (10 < ++b); // 左が false → 右は評価されない → b は 10 のまま
```

---

**項目**

演算子 — 優先順位と左からの評価（`Chapter3_answers.md` 設問6）

**題目**

`%` `*` `/` と `+` が混ざった式の順序を取り違えないため

**コツ**

カッコ・単項・インクリメントの次に、**乗除剰余**が **加減**より先。同じ優先度は**左から**評価。迷ったら一度 **`()` で意図どおりに分解**してから数値を代入する。

**例**

```java
int a = 100, b = 20, c = 30;
// (a % b * c) + (a / b) → (0 * 30) + 5 → 5
int r = (a % b * c) + (a / b);
System.out.println(r); // 5
```

---

**項目**

演算子 — カッコの後と **int 除算の切り捨て**（`Chapter3_answers.md` 設問7）

**題目**

`12 / 10` を「1.2」と読んでしまわないため

**コツ**

オペランドが **`int` のまま**なら除算結果も **`int`（小数切り捨て）**。**`()`** があればその中から先に評価する。

**例**

```java
int result = 30 - 12 / (2 * 5) + 1;
// (2*5)=10 → 12/10=1（int）→ 30-1+1=30
System.out.println(result); // 30
```

---

**項目**

演算子 — ビット演算子 `&` `|` `^`（`Chapter3_answers.md` 設問12）

**題目**

2 進ビット列を与えられたとき、結果がすべて 0 になる演算を選び間違えないため

**コツ**

桁ごとに表を書く。**AND** は両方 1 のところだけ 1。**OR** はどちらか 1 なら 1。**XOR** は片方だけ 1 なら 1。`1011` と `0100` で全体が 0 になるのは **AND のみ**（解答の典型）。

**例**

```java
//   1011
// & 0100
// = 0000

int x = 0b1011 & 0b0100;
System.out.println(x); // 0
```

---

**項目**

参照と比較 — **同一性** `==`（`Chapter3_answers.md` 設問8）

**題目**

`new` や代入のあとに **`==` が true か false か**を取り違えないため

**コツ**

参照型の **`==` は「同じインスタンスを指すか」**だけ。`s2 = s1` で共有しても、**片方だけ `new` し直す**と別インスタンスになり **`false`**。

**例**

```java
Object s1 = new Object();
Object s2 = s1;
System.out.println(s1 == s2); // true
s1 = new Object();
System.out.println(s1 == s2); // false
```

---

**項目**

`Object#equals` — **オーバーライド**と比較に使うフィールド（`Chapter3_answers.md` 設問9）

**題目**

「どのフィールドが一致すれば `equals` が true か」を読み落とさないため

**コツ**

`equals` はクラス設計者が定義した**同値条件**に従う。設問のように **num だけ見る**なら `name` が違っても **`true`** になり得る。`==` とは役割が違う。

**例**

```java
// 概念例: num のみで equals を定義したクラス
// a.equals(b) が true でも a == b は false（別インスタンス）になり得る
```

---

**項目**

`Object#equals` — **オーバーロード**とシグネチャ（`Chapter3_answers.md` 設問10）

**題目**

`equals(Sample obj)` を書いたつもりで **`equals(Object)` が呼ばれる**パターンを踏まないため

**コツ**

**オーバーライド**は `boolean equals(Object obj)` と**引数型まで一致**させる。引数だけ変えた同名メソッドは**オーバーロード**。`a.equals(b)` で実際に選ばれるのは **`Object` 版**になり、デフォルトは **`this == obj`** 相当 → 別インスタンスなら **`false`**。

**例**

```java
// class Bad {
//   public boolean equals(Bad other) { ... } // オーバーロード（Object 版はそのまま）
// }
// Bad a = new Bad(), b = new Bad();
// a.equals(b); // 実際は Object#equals が動き false になりがち
```

---

**項目**

`Object#equals` — **`null` 引数**（`Chapter3_answers.md` 設問11）

**題目**

`x.equals(null)` の戻り値を取り違えないため

**コツ**

**null でない参照 `x` について `x.equals(null)` は `false`**（契約）。`Object` の実装は `return (this == obj);` のため、`obj` が `null` なら `false`。

**例**

```java
String s = "a";
System.out.println(s.equals(null)); // false
// 実行時 NPE を避けるなら Objects.equals(a, b) や "literal".equals(var) を使う
```

---

**項目**

制御構文 — **`if` の構文**（`Chapter3_answers.md` 設問13）

**題目**

**条件式の `()`** や **`then` の有無**など、他言語と混同しないため

**コツ**

Java の **`if` は必ず `if (条件式)`**。**`then` `end if` はない**。条件のカッコ省略や `then` 付きは**誤り肢**として覚える。

**例**

```java
if (true) { /* ... */ }
```

---

**項目**

制御構文 — `if` / `else` の **中カッコ省略**（`Chapter3_answers.md` 設問14・15）

**題目**

「`if` にぶら下がるのは次の 1 文だけ」を見失わないため

**コツ**

`{}` を省略したら、**直後の 1 文だけ**が `if` または `else` の本体。**インデントは無意味**。試験では脳内で **`{}` を補完**してから出力順を追う。

**例**

```java
if (false)
    System.out.println("A");
System.out.println("B"); // 常に実行（if の外）
// 出力: B
```

---

**項目**

制御構文 — **`if` / `else` のあとに続く別の `if`**（`Chapter3_answers.md` 設問16）

**題目**

`else if` ではなく**独立した 2 つ目の `if`**だと表示がどうなるか取り違えないため

**コツ**

1 つ目の `if` / `else` が終わったあと、**次の `if` は別の文**。それぞれの条件を**別々に**評価する。

**例**

```java
int num = 10;
if (num < 10)
    System.out.println("A");
else
    System.out.println("B");
if (num == 10)
    System.out.println("C");
// 出力: B 改行 C
```

---

**項目**

`switch` — **条件式に使える型**（`Chapter3_answers.md` 設問17）

**題目**

「整数なら全部 `switch` できる」と **`long` や `boolean`** を含めてしまわないため

**コツ**

覚え方の一例: **`int` 以下の整数＋ラッパー**、**`String`**、**`enum`**。解答が明示するように **`long` / `boolean` / 浮動小数**は条件式に**使えない**。

**例**

```java
// long x = 1L;
// switch (x) { } // NG

int y = 1;
switch (y) { case 1: break; } // OK
```

---

**項目**

`switch` — **`case` ラベルは定数**（`Chapter3_answers.md` 設問18）

**題目**

変数や型不一致の `case` をコンパイルエラーと見抜けないため

**コツ**

`case` には**コンパイル時定数**（リテラル、`final` 定数、定数式など）。**普通の変数**は `case` に**書けない**。条件式の型と **`case` の型**も揃える。

**例**

```java
final int A = 1;
int x = 1;
switch (x) {
    case A: break;      // OK（final 定数）
    // case x: break; // NG（変数）
}
```

---

**項目**

`switch` **文** — **フォールスルー**（`Chapter3_answers.md` 設問19）

**題目**

`break` がないときに **下の `case` まで実行が流れる**のを忘れないため

**コツ**

**`switch` 文**では `break` が現れるまで**下へ落ちる**（フォールスルー）。表示順・実行順を**上から積み上げ**て追う。

**例**

```java
int num = 10;
switch (num) {
    case 10: System.out.print("A");
    case 11: System.out.print("B");
    default: System.out.print("C");
}
// break なし → ABC
```

---

**項目**

`switch` **文** — **`default` の位置**とフォールスルー（`Chapter3_answers.md` 設問20）

**題目**

`default` が **`case` より上**にあっても、**下の `case` に流れる**ことを見落とさないため

**コツ**

`default` は「どの `case` にも当てはまらないとき」に入るが、**そこに `break` がなければ**その下の `case` にも流れる。文法上 `default` は**最後でなくてよい**。

**例**

```java
int n = 0;
switch (n) {
    default:
        System.out.print("C");
        // break なし
    case 1:
        System.out.print("A");
        break;
}
// default → case 1 へフォールスルーし A まで → CA
```

---

**項目**

`switch` **式** — **`case ... ->` とフォールスルー**（`Chapter3_answers.md` 設問21）

**題目**

**矢印形式**の `switch` で、従来の **`:` + `break` 忘れ**と同じ挙動だと思い込まないため

**コツ**

**`case 値 -> 文/式`** は **その arm だけ実行して終わる**（**暗黙で分岐が閉じる**）。**`break` は不要**で、**下の `case` には落ちない**。従来の **`:` 形式の `switch` 文**のフォールスルーと**対比**して覚える。

**例**

```java
int i = 2;
switch (i) {
    case 1 -> System.out.println("A");
    case 2 -> System.out.println("B"); // B のみ
    default -> System.out.println("C");
}
```

---

**項目**

`switch` **式** — **網羅と `default`**（`Chapter3_answers.md` 設問22）

**題目**

**式**として値を返す `switch` で、**`default` 不足がコンパイルエラー**になる条件を取り違えないため

**コツ**

**`switch` 式**はすべての入力がどこかの arm に落ちる必要がある。**`int` の全域**を少数の `case` で埋められないときは **`default` が実質必須**になりやすい。従来の **`switch` 文**より**厳しい**、と対比して覚える。

**例**

```java
int x = 1;
// int v = switch (x) { case 1 -> 10; case 2 -> 20; }; // int の残りが無いとコンパイルエラー
int v = switch (x) {
    case 1 -> 10;
    case 2 -> 20;
    default -> 0;
};
```

---

**項目**

`switch` — **文と式の本質的な違い**（`Chapter3_answers.md` 設問23）

**題目**

**矢印かコロンか**だけが違い、と誤解しないため

**コツ**

見た目より重要なのは **`:`/`->` より「文か式か」**。**`switch` 文**は手続きを書き、**値を返さない**。**`switch` 式**は**値を生成**し代入の右辺などに使える。**構文要素の違いは結果として現れる**が、本質は**式か文か**。

**例**

```java
int x = 1;
int v = switch (x) { case 1 -> 10; default -> 0; }; // 式: 値になる
// switch (x) { case 1: System.out.println(1); } // 文: そのままでは int へ代入できない
```

---

**項目**

`switch` **式** — **`->` の右に書けるもの**（`Chapter3_answers.md` 設問24）

**題目**

矢印の右に **`return`** で値を返せる、と誤らないため

**コツ**

**`->` の右**は次のいずれか:**値を返す式**、**`throw`**、**`{ ... yield 値; }` 形式のブロック**。**`return`** は**メソッドから抜ける**だけで、**`switch` 式の結果にはならない**（式の値が欲しいときは **`yield`**）。

**例**

```java
int x = 1;
int v = switch (x) {
    case 1 -> 10;
    case 2 -> { yield 20; }
    default -> 0;
};
```

---

**項目**

繰り返し — **`while` の条件式**（`Chapter3_answers.md` 設問25）

**題目**

**真偽値 1 つ**と、**回数・表示範囲**（例: 0〜4）を対応づけて選び間違えないため

**コツ**

**`while (条件)` の条件は必ず `boolean`**。**回数や変化**（0,1,2,3,4 を出す等）に合わせて **`<` `<=` `>` `>=`** を選ぶ。**`true`/`false` リテラル**は無限ループ・0 回の典型肢。

**例**

```java
int b = 0;
while (b < 5) {
    System.out.println(b);
    b++;
}
```

---

**項目**

繰り返し — **`do`-`while` の形と最低 1 回実行**（`Chapter3_answers.md` 設問26）

**題目**

`do` の直後に `while (条件)` を書く誤りや、**セミコロン**を忘れないため

**コツ**

形は **`do { } while (条件);`**。`**`do` の直後に条件を書かない**。**`while` の後は `;` 必須**。**本体は先に実行**され、そのあと条件判定（`while` と逆）。

**例**

```java
int i = 0;
do {
    System.out.println(i);
    i++;
} while (i < 3);
```

---

**項目**

繰り返し — **`do`-`while` と中カッコ省略**（`Chapter3_answers.md` 設問27）

**題目**

`do` と `while` の間に**文が 2 つ以上**あるのに `{}` がないコードをコンパイルエラーと見抜くため

**コツ**

`if` と同様、省略時は **`do` の直後の 1 文だけ**がループ本体。**2 文以上**なら **`{}` 必須**。

**例**

```java
// do
//     System.out.println("A");
//     System.out.println("B");
// while (false); // NG（中カッコなしで 2 文）

do {
    System.out.println("A");
    System.out.println("B");
} while (false); // OK
```

---

**項目**

繰り返し — **`for` の初期化文**（`Chapter3_answers.md` 設問28）

**題目**

初期化で **`int` と `long` をカンマ区切り**で同時宣言してしまわないため

**コツ**

初期化の **`int a = 0, b = 1`** のように、**同じ型**だけをカンマで並べられる。**型が混ざる**複数宣言は **NG**。

**例**

```java
// for (int i = 0, long j = 0; i < 1; i++) {} // NG
for (int i = 0, j = 0; i < 1; i++) {} // OK
```

---

**項目**

繰り返し — **`for` 初期化で宣言した変数のスコープ**（`Chapter3_answers.md` 設問29）

**題目**

ループ変数を **`for` の外**で使えると誤解しないため

**コツ**

**初期化で宣言した変数**は **`for` のヘッダと本体ブロック内だけ**有効。**ループのあと**で参照すると**コンパイルエラー**。必要なら**ループの外で先に宣言**する。

**例**

```java
for (int i = 0; i < 3; i++) { }
// System.out.println(i); // NG
int j;
for (j = 0; j < 3; j++) { }
System.out.println(j); // OK（ヘッダで型宣言していない場合は別）
```

---

**項目**

繰り返し — **`for` の条件文の評価タイミング**（`Chapter3_answers.md` 設問30）

**題目**

**「本体の前に条件」**と**「更新のあとに条件」**を取り違えないため

**コツ**

1 周目も含め、**本体の直前**に必ず**条件文**が評価される。流れは **初期化 → 条件 →（true なら）本体 → 更新 → 条件 → …**。**条件が先**と固定して表を書く。

**例**

```java
// i=0 で一度も本体が走らない例: for (int i = 0; i < 0; i++) { ... }
```

---

**項目**

繰り返し — **`for` の条件文にカンマは使えない**（`Chapter3_answers.md` 設問31）

**題目**

初期化や更新と同様に、**条件をカンマで並べる**誤りをしないため

**コツ**

**カンマで複数書けるのは初期化文と更新文**。**条件文は 1 式**。**複数条件**は **`&&` / `||` で 1 つの `boolean` 式**にまとめる。

**例**

```java
for (int i = 0, j = 0; i < 3 && j < 5; i++, j++) { }
```

---

**項目**

繰り返し — **`for` の更新文の実行順**（`Chapter3_answers.md` 設問32）

**題目**

**本体と更新**のどちらが先か、**カンマやメソッド呼び出し**の表示順を取り違えないため

**コツ**

各反復で **条件（true）→ 本体 → 更新**の順。**更新文**はカンマで並べた場合も**左から順に**評価されることが多いが、設問は**「本体のあとに更新」**が軸。表示問題はこの順に**1 行ずつ書き出す**。

**例**

```java
for (int i = 0; i < 3; i++, System.out.print(",")) {
    System.out.print(i);
}
// 典型: 0,1,2, のように「値→カンマ」が交互（設問の選択肢に合わせて最終行のカンマに注意）
```

---

**項目**

配列 — **二重ループと `array[i].length`**（`Chapter3_answers.md` 設問33）

**題目**

ジグザグ配列で **行ごとの長さ**を固定の数で代用してしまわないため

**コツ**

行 `i` の列数は **`array[i].length`**。内側は原則 **`j < array[i].length`**。固定の `j < 2` などは**合計がずれる**か **範囲外**になりやすい。

**例**

```java
int[][] a = { {1, 2}, {2, 3, 4} };
// 内側は必ず array[i].length 未満まで回す（行ごとの列数が違う）
for (int i = 0; i < a.length; i++) {
    for (int j = 0; j < a[i].length; j++) {
        // a[i][j] を処理
    }
}
```

---

**項目**

配列 — **`length` は 1 次元目か行か**（`Chapter3_answers.md` 設問34）

**題目**

**`matrix.length`** と **`matrix[i].length`** を混同しないため

**コツ**

**`matrix.length`** は **行数（1 次元目の要素数）**。**`matrix[i].length`** は **その行の列数**。二重ループの上限がどちらの `length` を見ているか、**変数名ごとに分離**して書き出す。

**例**

```java
String[][] m = new String[2][];
m[0] = new String[2];
m[1] = new String[2];
System.out.println(m.length);    // 2（行）
System.out.println(m[0].length); // 2（その行の列）
```

---

**項目**

繰り返し — **無限ループの条件**（`Chapter3_answers.md` 設問35）

**題目**

**「終わらない繰り返し」**になるパターンを選び漏れ／選び過ぎしないため

**コツ**

代表例:**`while (true)`**、**`for (;;)`**（条件省略）、**`for (...; true; ...)`**。**`break` 等がなければ**停止しない。**条件が常に false** は **0 回**。設問が「N 個選べ」なら、**個数と集合**を最後に照合する（設問2・17・18・35・39 などと同型の落とし穴）。

**例**

```java
// while (true) { }              // 無限（break がなければ止まらない）
// for (;;) { }                 // 条件省略も同様
// for (int i = 0; true; i++) { }
```

---

**項目**

繰り返し — **拡張 `for` の構文と 2 次元配列**（`Chapter3_answers.md` 設問36）

**題目**

**`:` の左右**や、**2 次元配列の要素の型**を取り違えないため

**コツ**

形は必ず **`for (型 変数 : 集合)`**（**集合 `:` 変数**は不可）。**2 次元配列**を回すと、各要素は**行の配列**（例: `int[]`）であり、**`Object` で受けられる**（配列はオブジェクト）。**既存変数名を一時変数に使い回す**のは **NG** になりやすい。

**例**

```java
int[][] a = {{1,2},{3}};
for (int[] row : a) { /* ... */ }
for (Object row : a) { /* ... */ }
```

---

**項目**

繰り返し — **拡張 `for` の一時変数と集合**（`Chapter3_answers.md` 設問37）

**題目**

一時変数へ代入したつもりで**配列の中身まで変わる**と誤解しないため

**コツ**

拡張 `for` の変数は**要素（参照）のコピー**。**変数に別オブジェクトを代入**しても**配列のスロットは差し替わらない**。**ミュータブルオブジェクトの内部変更**は配列に影響する、という対比で覚える。

**例**

```java
String[] a = {"A","B","C"};
for (String s : a) {
    s = "D"; // 各要素は A,B,C のまま（String はイミュータブルだが、参照の差し替えが配列に届かないのが要点）
}
```

---

**項目**

繰り返し — **`while` と `do-while` の評価順**（`Chapter3_answers.md` 設問38）

**題目**

**同じインクリメント**でも、`while` の条件に載せるか `do` ブロックに載せるかで**結果が変わる**選択肢を機械的に当てはめないため

**コツ**

**`do-while` は本体のあとに条件**。**`while` は本体の前に条件**。**前置／後置**と組み合わせると差が出るので、**小さい表**（`num` と表示・回数）で追う。**「A〜D はすべて誤り」**肢が出たら、**消去法**も検討する。

**例**

```java
// 典型: 同じ更新でも while と do-while で「最初の 1 回」が変わる
```

---

**項目**

配列 — **添字とデクリメント・境界**（`Chapter3_answers.md` 設問39）

**題目**

**長さ `length` と有効添字 `0..length-1`** の関係で **`ArrayIndexOutOfBoundsException`** を選び抜くため

**コツ**

**最初の反復で触る添字**が **`length-1` 以下か**を必ず確認。**`x` を減らしながら `arr[x]`** なら、**減らす前／後**のどちらでアクセスするかで**範囲外**になりやすい。**正しいコードが複数肢に重複**している場合もあるので、**完全一致**で選ぶ。

**例**

```java
int[] a = {1,2,3,4,5};
int x = a.length;
while (x > 0) {
    x--;
    System.out.print(a[x]); // 5..1
}
```

---

**項目**

繰り返し — **`break` と二重ループ**（`Chapter3_answers.md` 設問40）

**題目**

**内側だけ**抜けて外側は回る、という挙動で表示回数を取り違えないため

**コツ**

**`break` は最も内側の `switch` またはループだけ**を抜ける。外側まで一気に抜けたいときは**ラベル付き `break`** など別手段。

**例**

```java
outer:
for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 3; j++) {
        if (j == 1) break; // 内側だけ抜ける
        System.out.print(i + "" + j + " ");
    }
}
// 内側 j=0 のみ各 i で実行 → 00 10 のようなパターン（設問に合わせて表で追う）
```

---

**項目**

繰り返し — **`continue` と `break` の違い**（`Chapter3_answers.md` 設問41）

**題目**

**「その回の残りを飛ばす」**のか**「ループを抜ける」**のかを取り違えないため

**コツ**

**`continue`**: その反復の**残りの文をスキップ**し、**次の条件判定**へ（ループは続く）。**`break`**: **ループ全体を脱出**。**偶数を飛ばして加算**などは、`i` の表に「スキップ／加算」を書き込む。

**例**

```java
int total = 0;
for (int i : new int[]{1,2,3,4,5}) {
    if (i % 2 == 0) continue;
    total += i; // 1+3+5
}
```

---

**項目**

制御構文 — **ラベルを付けられる構文**（`Chapter3_answers.md` 設問42）

**題目**

ラベルは**二重ループ専用**だと思い込まないため

**コツ**

**`break ラベル` / `continue ラベル`** で飛び先を指定できるのはよく出るが、**ラベル自体**は**ループに限らない**。**ブロック、`if`、`switch`、式、代入、`return`、`try`、`throw`** などにも付けられる、という知識問題では**「すべて該当」**肢に注意する（可読性は別問題）。

**例**

```java
label: {
    if (true) break label;
}
```

---

**項目**

繰り返し — **ラベル付き `continue` / `break`**（`Chapter3_answers.md` 設問43）

**題目**

**どのループの次周回／どこへ抜けるか**を、`i` と `j` の表で追わずに誤らないため

**コツ**

**`continue ラベル`** はそのラベルの**外側ループの次反復**へ。**`break ラベル`** はそのラベルが付いた**文の直後**へ。二重以上は**表**（`i`×`j`）に「continue」「break」「加算」を書き込む。

**例**

```java
int total = 0;
outer:
for (int i = 0; i < 3; i++) {
    inner:
    for (int j = 0; j < 3; j++) {
        if (i == 0) continue outer; // 外側の次の i へ
        if (j == 2) break inner;    // inner ラベルの直後＝内側ループを抜ける
        total += i + j;
    }
}
// 実際の値は設問のコードに合わせて表で検算する（解答は total=12 系の典型）
```

---

**項目**

答案の作法 — **複数選択の完全一致**（設問2・17・18・35・39 など。採点は `Chapter3_sec_scoring_20260513.md` 参照）

**題目**

正解の**個数と集合**が公式と一致しているか、マークシートで取り違えないため

**コツ**

**1 つでも多い／少ない／違う肢があると不正解**になりやすい。問題文の **「N つ選択」**と、自分が選んだ**記号の個数**を最後に必ず突き合わせる（知識は合っていてもマークミスで落ちるのを防ぐ）。

**例**

```
正解が {A,C,D} のとき:
  {A,C}   → 不足で不正解
  {A,C,D,B} → 余分で不正解
  {A,C,D} → 順不同で可（採点ルールは採点結果ファイルの通り）
```

---

以上。
